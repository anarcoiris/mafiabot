...
VERSIÓN CORREGIDA - Indentación arreglada
"""
...
class GameView(ttk.Frame):
    ...
    def _create_right_column(self, parent):
        """Crea la columna derecha con log y muertos."""
    ...
    def _create_player_card(self, parent, player):
        ...
        states = []
        if getattr(player, "silenced", False):
            states.append("🤐 Silenciado")
        if getattr(player, "blocked", False):
    ...
    def _on_destroy(self, event):
        """Cleanup cuando la vista se destruye."""
    ...
    def _cleanup_controller_callbacks(self):
        """Limpia los callbacks del controller."""
        try:
